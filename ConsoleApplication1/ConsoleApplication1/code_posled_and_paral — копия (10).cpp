...
unsigned fib_async(unsigned n) {
	if (n < 2) return n;
	packaged_task t1([n]() {return fib_async(n - 1); });
	auto fibn1 = std::async([&t1]() {t1(); return t1.get_future().get(); });
	auto fibn2 = std::async([] (unsigned n) { return fib_async(n - 2); }, n);
	return fibn1.get() + fibn2.get();
}
...
int main() {
	std::cout << "fib_async(13):" << std::endl;
	std::cout << fib_async(13) << std::endl;
...
// эта реализация не работает тут!
...
// подключаем библиотеку future

unsigned fib_async (unsigned n) {
	if (n < 2) return n;
	packaged_task t1([n]() { return fib_async (n-1); });
	auto fibn1 = async(t1);
	auto fibn2 = async([] (unsigned n) { return fib_async(n-2); }, n);
	return fibn1.get() + fibn2.get();
}
